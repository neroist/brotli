...
const
  simpl = "lazy test daaaaaataaaaaaa ssddddvfggggfdks meow :3c"
  simplCompressed =
...
suite "encoding":
  test "simple test":
    let compressed = compressBrotli(simpl)
    check compressed == simplCompressed

suite "decoding":
  test "simple test":
    let decompressed = decompressBrotli(simplCompressed)
    check decompressed == simpl
